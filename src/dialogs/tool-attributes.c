...
sp_tool_attributes_dialog (void)
{
	...
	if (!dlg) {
		...
		gtk_window_set_title (GTK_WINDOW (dlg), _("Tool attributes"));
		gtk_signal_connect (GTK_OBJECT (dlg), "destroy", GTK_SIGNAL_FUNC (sp_tool_attributes_dialog_destroy), NULL);

		sp_tool_attributes_dialog_setup (ec);

		gtk_signal_connect (GTK_OBJECT (SODIPODI), "set_eventcontext", GTK_SIGNAL_FUNC (sp_tool_attributes_dialog_set_eventcontext), dlg);
