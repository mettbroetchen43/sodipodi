...
static GtkWidget *layout_dialog = NULL;

static void
sp_object_layout_dialog_destroy (GtkObject *object, gpointer data)
{
	layout_dialog = NULL;
}
...
sp_object_layout_any_value_changed (GtkAdjustment *adj, SPWidget *spw)
{
	...
	if ((fabs (x0 - bbox.x0) > 1e-3) ||
		...
		nr_matrix_f_set_scale (&scale, (x1 - x0) / (bbox.x1 - bbox.x0), (y1 - y0) / (bbox.y1 - bbox.y0));
		nr_matrix_f_set_translate (&o2n, x0, y0);
		nr_matrix_multiply_fff (&s , &p2o, &scale);
		nr_matrix_multiply_fff (&t , &s, &o2n);
		/* No duplicate, not repeatable */
		sp_selection_apply_affine (sel, &t, FALSE, FALSE);
