...
/*
 * Specialization of GtkText for the XML tree view
 *
 * Authors:
...
static void sp_xmlview_content_changed (GtkEditable * editable);
...
sp_xmlview_content_new (SPRepr * repr)
{
	...
	gtk_text_set_adjustments (GTK_TEXT (text), GTK_ADJUSTMENT (gtk_adjustment_new (0.0, 0.0, 1.0, 0.2, 1.0, 1.0)), GTK_ADJUSTMENT (gtk_adjustment_new (0.0, 0.0, 1.0, 0.2, 1.0, 1.0)));
...
sp_xmlview_content_set_repr (SPXMLViewContent * text, SPRepr * repr)
{
	...
	if (repr) {
		...
		gtk_editable_set_editable (GTK_EDITABLE (text), 0);
	}
}
...
sp_xmlview_content_get_type (void)
{
	...
	if (!type) {
		...
		type = gtk_type_unique (GTK_TYPE_TEXT, &info);
	}
...
sp_xmlview_content_class_init (SPXMLViewContentClass * klass)
{
	...
	editable_class->changed = sp_xmlview_content_changed;

	parent_class = gtk_type_class (GTK_TYPE_TEXT);
}
...
void
sp_xmlview_content_changed (GtkEditable * editable)
{
	SPXMLViewContent * text;
	text = SP_XMLVIEW_CONTENT (editable);
