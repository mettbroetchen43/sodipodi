		   ...
		   ArtPoint *p)
{
	...
	p->x = star->r1 * cos(star->arg1) + star->cx;
	p->y = star->r1 * sin(star->arg1) + star->cy;
}
		   ...
		   ArtPoint *p)
{
	...
	p->x = star->r2 * cos(star->arg2) + star->cx;
	p->y = star->r2 * sin(star->arg2) + star->cy;
}
...
void
sp_star_build_repr  (SPStar *star,
		     SPRepr *repr)
{
	g_return_if_fail (star != NULL);
